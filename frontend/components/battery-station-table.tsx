...
export function BatteryStationTable({ batteryStations = {} }: BatteryStationTableProps) {
            ...
            {filteredStations.length === 0 ? (
              ...
              filteredStations.map((station) => {
                const usagePercentage = Math.round(
                  ((station.total_capacity - station.available_batteries) / station.total_capacity) * 100,
                )
                ...
                return (
                  <TableRow key={station.id}>
                    ...
                    <TableCell className="text-green-600 font-medium">{station.available_batteries}</TableCell>
                    <TableCell className="text-orange-600 font-medium">{station.charging_batteries}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Progress value={usagePercentage} className="h-2 w-20" />
                        <span className="text-sm">
                          {station.total_capacity - station.available_batteries}/{station.total_capacity}
                        </span>
                      </div>
                    </TableCell>
                  </TableRow>
