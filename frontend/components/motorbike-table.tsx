...
export function MotorbikeTable({ motorbikeStates = {} }: MotorbikeTableProps) {
  ...
  const safeString = (value: any, defaultValue = ""): string => {
    return value && typeof value === "string" ? value : String(value || defaultValue)
  }
    ...
    <div className="space-y-4">
          ...
          <TableBody>
            {filteredMotorbikes.length === 0 ? (
              ...
              filteredMotorbikes.map((motorbike, index) => {
                ...
                const batteryPercentage = batteryMax > 0 ? Math.round((batteryNow / batteryMax) * 100) : 0
                const motorbikeId = safeString(motorbike.id, `MB${index + 1}`)
                const status = safeString(motorbike.status, "unknown")
                const onlineStatus = safeString(motorbike.online_status, "unknown")
                  ...
                  <TableRow key={motorbikeId}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Progress value={batteryPercentage} className="h-2 w-20" />
                    ...
                    <TableCell>
                      <Badge variant={onlineStatus === "online" ? "default" : "destructive"}>{onlineStatus}</Badge>
                    </TableCell>
                    <TableCell>
                      {motorbike.assigned_order_id && (
                        <span className="text-sm">Order: {safeString(motorbike.assigned_order_id)}</span>
                      )}
                      {motorbike.assigned_station_id && (
                        <span className="text-sm">Station: {safeString(motorbike.assigned_station_id)}</span>
                      )}
                      {!motorbike.assigned_order_id && !motorbike.assigned_station_id && (
      ...
      {process.env.NODE_ENV === "development" && (
        <div className="mt-4 p-2 bg-gray-100 rounded text-xs">
          <strong>Debug:</strong> Found {Object.keys(motorbikeStates).length} motorbike entries, {motorbikes.length}{" "}
          valid motorbikes, {filteredMotorbikes.length} after filtering
        </div>
      )}
    </div>
  )
