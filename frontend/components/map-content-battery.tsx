...
export default function MapContentBattery({
  ...
  const [selectedMotorbike, setSelectedMotorbike] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    Object.entries(motorbikeStates).forEach(([motorbikeId, state]) => {
      ...
      const { current_lat, current_lon, status, battery_now, battery_max } = state
      ...
      if (motorbikeMarkersRef.current[motorbikeId]) {
        ...
        motorbikeMarkersRef.current[motorbikeId].setLatLng([current_lat, current_lon])
        motorbikeMarkersRef.current[motorbikeId].setIcon(motorbikeIcon)
      } else {
        // Create new marker
        const marker = L.marker([current_lat, current_lon], { icon: motorbikeIcon })
          .addTo(map)
          .on("click", () => {
            setSelectedMotorbike(selectedMotorbike === motorbikeId ? null : motorbikeId)
          })
                ...
                <Badge
                  className={
                    motorbikeStates[selectedMotorbike]?.status === "idle"
                      ? "bg-blue-500"
                      : motorbikeStates[selectedMotorbike]?.status === "on_order"
                        ? "bg-green-500"
                        : motorbikeStates[selectedMotorbike]?.status === "heading_to_station"
                          ? "bg-orange-500"
                          : motorbikeStates[selectedMotorbike]?.status === "charging"
                            ? "bg-red-500"
                            : "bg-gray-500"
                  }
                >
            ...
            <CardContent className="py-3">
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{motorbikeStates[selectedMotorbike]?.online_status}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-medium">Latitude:</span>
                  <span>{motorbikeStates[selectedMotorbike]?.current_lat.toFixed(6)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-medium">Longitude:</span>
                  <span>{motorbikeStates[selectedMotorbike]?.current_lon.toFixed(6)}</span>
                </div>
                {motorbikeStates[selectedMotorbike]?.assigned_order_id && (
                  <div className="flex justify-between">
                    <span className="font-medium">Order ID:</span>
                    <span>{motorbikeStates[selectedMotorbike]?.assigned_order_id}</span>
                  </div>
                )}
                {motorbikeStates[selectedMotorbike]?.assigned_station_id && (
                  <div className="flex justify-between">
                    <span className="font-medium">Station ID:</span>
                    <span>{motorbikeStates[selectedMotorbike]?.assigned_station_id}</span>
                  </div>
                )}
              </div>
              <Button variant="outline" size="sm" className="w-full mt-3" onClick={() => setSelectedMotorbike(null)}>
                Close
              </Button>
            </CardContent>
          </Card>
