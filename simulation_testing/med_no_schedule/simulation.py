...
class Simulation:
    ...
    def calculate_final_metrics(self):
        ...
        if self.waiting_time_tracking:
            ...
            median_waiting_time = statistics.median(self.waiting_time_tracking)
        else:
            avg_waiting_time = 0
...
def run_multiple_simulations(num_drivers, num_stations, csv_path, num_runs=3):
    ...
    for run in range(num_runs):
        ...
        print(f"  Median Waiting Time: {result['median_waiting_time']:.2f} minutes")
    
    return results

def generate_analysis_graphs(results):
    """Generate comprehensive analysis graphs"""
    metrics = ['avg_operating_profit', 'num_drivers_waiting', 'avg_waiting_time', 'median_waiting_time']
    titles = [
        'Average Operating Profit of Drivers',
        'Number of Drivers Waiting at Swap Stations',
        'Average Waiting Time of Drivers at Swap Stations (minutes)',
        'Median Waiting Time of Drivers at Swap Stations (minutes)',
    ]
